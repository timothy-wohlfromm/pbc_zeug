...
def correlation(function, frames): 
    ...
    ret = map(lambda f: function(start_frame, f).mean(axis=0), itertools.chain([start_frame], iterator))
...
def subensemble_correlation(selector_function):
    ...
    def c(function, frames):
        ...
        def cc(act_frame): 
            ...
            full = function(start_frame, act_frame)
            for index in np.ndindex(sel_shape):
                sel = selectors[index]
                if is_first_frame_loop:
                    count[index]    = len(start_frame[sel])
                f_values[index] = full[sel].mean(axis=0) if count[index] != 0 else 0
